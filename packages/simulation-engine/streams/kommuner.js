...
const { map, take, filter, toArray, concatMap, tap, last, mergeAll } = require('rxjs/operators')
...
async function read() {
  ...
  return from(data).pipe(
    ...
    map((kommun) =>
      ({
        ...kommun,
        squares: squares.filter(({position: {lon, lat}}) => inside([lon, lat], kommun.geometry.coordinates[0]))
      })
    )
  )
}
