...
const $finder = newBookings
  ...
  .take(3)
  .tap((booking) =>
    console.log(`#${booking.id} - new booking. Looking for cars...`)
  )
  .doto((booking) => {
    booking.$fastestCars = carFinder(booking)
  })
  // .filter(booking => moment(booking.pickupDate).subtract(5, 'minutes').isAfter(new Date()))
  .tap((booking) => {
    booking.$fastestCars.observe().each((hit) =>
      console.log(`===========
