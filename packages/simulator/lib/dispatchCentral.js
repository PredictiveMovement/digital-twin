...
const { toArray, map, tap, filter, takeUntil, delay, mergeMap, catchError } = require('rxjs/operators')
...
const dispatch = (vehicles, bookings) => {
  return bookings.pipe(
    ...
    mergeMap((booking) => vehicles.pipe(
      ...
      map((vehicles) => vehicles.sort((a, b) => a.distance - b.distance).shift()?.car),
      filter(car => car),
