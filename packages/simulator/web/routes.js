...
const cleanBookings = () => // TODO: Replace cleanBookings with .toObject() on Booking
  pipe(
    map(
      ({
        pickup,
...
const cleanCars = ({ // TODO: Replace cleanCars with .toObject() on Vehicle
  position: { lon, lat },
  id,
  altitude,
  heading,
      ...
      .subscribe((bookings) => {
        if (bookings.length) {
          bookings.map((booking) => {
            console.log("booking update", booking.id, booking.status, booking?.passenger?.name)
          })
